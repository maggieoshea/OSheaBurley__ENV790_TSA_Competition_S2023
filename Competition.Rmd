...
# Shows a really high RMSE
```

#### Discussion of ARIMA with 2 Exogenous Variables (Temp and Humidity)
This model did not appear to fit the data well, both on the plots and through a quantitative evaluation with the RMSE and MAPE. Because of this, a full forecast was not performed. 
...
#Plot model + all observed data
autoplot(ts_daily_demand) +
  autolayer(temponly_forecast, series="ARIMA with Exogenous Variables",PI=FALSE) +
  ylab("Demand")
...
The MAPE and RMSE suggest that including temperature does not improve the model given that these values were higher than 
