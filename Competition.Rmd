...
ts_daily_temp <- msts(daily_temp$avg_t_ws23, 
                           seasonal.periods =c(7,365.25),
                           start=c(2005,01,01))
# Using temperature and humidity vars with highest correlation to demand
...
### NN3 Interpretation
THE RMSE is 1151.663 and the MAPE is 18.813. 
...
### NN5 Interpretation
THE RMSE is 2183.415 and the MAPE is 54.47. 
...
### NN5 Interpretation
THE RMSE is 1110.899 and the MAPE is 18.2.  
...
### NN6 Interpretation
THE RMSE is 1082.454 and the MAPE is 18.3.  
...
NN7_scores <- accuracy(NN_for7$mean, ts_demand_test)
```

### NN7 Interpretation
THE RMSE is 601.055 and the MAPE is 11.93.  
...
## Saving Code (don't want to delete, but not sure we need it)
...
### ARIMA with Fourier Terms Model 2
``` {r ARIMA, echo=TRUE, message=FALSE, warning=FALSE}
# First Model: 
...
#Forecast with ARIMA fit
#also need to specify h for fourier terms
ARIMA_Fourier_forecast2 <- forecast(ARIMA_Fourier_fit2,
                           xreg=fourier(ts_demand_train,
                                        K=c(2,2),
                                        h=365),
                           h=365
...
#Plot forecasting results
autoplot(ARIMA_Fourier_forecast2) + ylab("Demand")

#Plot model + all observed data
autoplot(ts_daily_demand) +
  autolayer(ARIMA_Fourier_forecast2, series="ARIMA_FOURIER",PI=FALSE) +
  ylab("Demand")

# Plot model and observed data during 2010
plot(ARIMA_Fourier_forecast2$mean,
     xlab = "Date",
     ylab = "Demand",
     ylim = c(1000, 10000))
lines(ts_demand_test,
...
ARIMA_four_scores2 <- accuracy(ARIMA_Fourier_forecast2$mean, ts_demand_test)
accuracy(ARIMA_Fourier_forecast2)

```

### ARIMA with Fourier - Training Data Interpretation
THE RMSE is 510.81 and the MAPE is 11.04 

### Modeling Full Data - ARIMA with Fourier
After using the training dataset, re-running with the full dataset to forecast into 2011:
```{r}
# First Model: 
ARIMA_Fourier_full2 <- auto.arima(ts_daily_demand, 
                             seasonal=FALSE, 
                             lambda=0,
                             xreg=fourier(ts_daily_demand, 
                                          K=c(2,2)) # 2 weekly cycles, 2 annual cycles
...
#Forecast with ARIMA fit
#also need to specify h for fourier terms
ARIMA_Fourier_FULLforecast2 <- forecast(ARIMA_Fourier_full2,
                           xreg=fourier(ts_daily_demand,
                                        K=c(2,2),
                                        h=n_forecast),
                           h=n_forecast) 
autoplot(ARIMA_Fourier_FULLforecast2) + ylab("Demand")

```

### Export Model Results
```{r}
ARIMA_fourier_export2 <- data.frame(date=forecast_dates, load=ARIMA_Fourier_FULLforecast2$mean)
write.csv(ARIMA_fourier_export2, "./Submissions/ARIMA_fourier_v3.csv", row.names = FALSE)
```

### Update from Maggie (notes to self and to group!): 
I made 3 new models with exogenous variables but for the future forecasting, I just used temperature and humidity values from the jan/feb of the year prior. A really complicated but maybe better model could forecast humidity and temperature using an ARIMA and then put those forecast values into the ARIMA for demand with exogenous variables. However, I think this could be a lot of work. Instead, we could do a few more exogenous variable ones but use different years for the forecast (instead of the temp/humidity for jan and feb 2010 we use 2009 for eaxample) *or* we could get the average temperature and humidity for each day and use that as the forecasted values. That may be the most fruitful but still less complicated than forecasting the values which is good! -- Not saying you have to do this, more making note for myself to remember my thought process! 

Next steps for either of us might include adding any other models that we want (right now we have 9 uploaded to the website), and then tidying up the code. Tidying the code I think will just involve making that table at the end with the final accuracy scores and ideally adding: message=FALSE, warning=FALSE to all code chunks (after r in {r}). Lastly, we can go through the notes in the code chunks that we left for each other and just keep those that make sense/explain our process. 