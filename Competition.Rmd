...
fullexog_arima <- auto.arima(ts_daily_demand, 
         ...
         xreg = cbind(fourier(ts_daily_demand, 
                                          K=c(2,6)),
               temperature = daily_temp$avg_t_ws23, 
               humidity = daily_humidity$rh_ws20))
```
...
temp_forecast <- forecast(ARIMA_Fourier_fit,
                           xreg=fourier(ts_demand_train,
                                        K=c(2,6),
                                        h=365),
                           h=365
...
temphumidity_forecast <- forecast(tempandhumidity,
                           xreg=cbind(fourier(ts_demand_train, 
                                          K=c(2,6)
                                         # ,h=365 --> I included this originally but it won't run with it here, so tagged it out. 
                                         ),
                                temperature = daily_temp$avg_t_ws23, 
                                humidity = daily_humidity$rh_ws20),
                           h=365) 
...
Although the plot appears to show that the lines match the observed data better, the MAPE and RMSE suggest that including temperature does not improve the model given that these values were higher than the RMSE/MAPE with both exogenous variables which already had high MAPE/RMSEs. 
...
#Plot model + all observed data
autoplot(ts_daily_demand) +
  autolayer(humidity_forecast, series="ARIMA with Exogenous Variables",PI=FALSE) +
  ylab("Demand")

accuracy(humidity_forecast)
# Shows the highest RMSE and MAPE compared to the other models with exogenous variables. 
```

#### ARIMA with Humidity Exogenous Discussion
The MAPE and RMSE suggest that including humidity, instead of temperature or without temperature, does not improve the model given that these values were higher than the RMSE/MAPE for both models with exogenous variables. 
